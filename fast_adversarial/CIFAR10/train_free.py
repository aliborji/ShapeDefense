...
def get_args():
    ...
    parser.add_argument('--epochs', default=5, type=int, help='Total number of epochs will be this argument * number of minibatch replays.')
    ...
    parser.add_argument('--minibatch-replays', default=2, type=int)
...
def main():
    ...
    model = define_model(args.net_type).cuda()
    model.train()
...
#    delta.requires_grad = True
    ...
    model_test = define_model(args.net_type).cuda()# PreActResNet18(args.net_type)#()    
    model_test.load_state_dict(model.state_dict())
    model_test.float()
    model_test.eval()

    pgd_loss, pgd_acc = evaluate_pgd(test_loader, model_test, 10, 10, args.net_type, args.redetect_test)
    #pgd_loss, pgd_acc = evaluate_pgd(test_loader, model_test, 10, 10)
